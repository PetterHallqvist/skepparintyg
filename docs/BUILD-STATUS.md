...
- **Phase 7 / M5** (next, last build phase): simulation blueprints, deterministic
  stratified assembly with exposure limits, normalized exam_sessions, server-
  authoritative timer, runner (autosave/flag/nav, no feedback until submit),
  scoring vs the configured 75 % rule, diagnostic sections. Labelled
  "Träningssimulering" with a non-official-format disclaimer.
