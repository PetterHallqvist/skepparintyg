...
## Done (all gates green: lint · typecheck · 250 unit tests · build)
...
| 9 | Certification tailoring — "rätt kort för rätt intyg" | `9898a59`…`016c274` (branch `claude/phase-9-cert-tailoring`) | The whole experience is scoped to the test the learner is taking (SPEC §7.1 — all 8 ids). `lib/certifications/registry.ts` = single source of truth (names, active/preview, curriculum modules §28–§33, tracks, chartLab, §17.1 weights). STRICT `(cert, track)` item registry — the silent Förarintyg fallback that leaked cross-cert questions is gone (pinned by isolation tests). Mandatory first-run picker `/app/valj-intyg` (httpOnly `sjoklart_intyg` cookie, proxy-gate, Byt intyg switchers), per-cert exams (`EXAM_CONFIG`, namespaced pool ids, cert-bound `sim_demo` + mismatch notice), nav/framsteg/kortlekar tailored (Sjökort↔Simulering tab swap). **~300-item original question bank** across all 8 certs (förar 61 incl. new regler track · kust 40 · src 40 · praktik/seglar 32–34), coverage mapped from public NFB exam structure, all review-status. Marketing: 8-cert homepage grid, seglarintyg-1..3 waitlist pages, per-cert free test `?intyg=` (explicit choice beats cookie). Migration 0009 `learners.target_certification_id` (§12.2) + pgTAP 0005, functionally verified on local PG17. |
