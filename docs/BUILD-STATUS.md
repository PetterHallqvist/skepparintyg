...
- **Phase 4 / M4**: rules-encounter trainer, lights/shapes/sound trainer (blink
  engine + WebAudio), knot trainer (needs original media, HV #11), remediation
  sessions, Anki/Quizlet export pipeline.
- **Phase 5 / M5**: simulation blueprints, deterministic assembly, server timer,
  runner, scoring vs configured 75 % rule.
- **Phase 6 / M6**: Stripe checkout + idempotent webhooks, entitlements/seats,
  guardian/minor flows, receipts, export/deletion. Needs accounts + price
  decision (HV #3, #14).
- **Phase 7 / M7**: full public routes, free diagnostic + chart demo, SEO,
